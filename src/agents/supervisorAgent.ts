...
`;
  ...
  private async sendMessage(agent: Agent, content: string, userId: string, sessionId: string, additionalParams: Record<string, any>): Promise<string> {
    try {
      if (this.trace) {
        Logger.logger.info(`\x1b[32m\n===>>>>> Supervisor sending ${agent.name}: ${content}\x1b[0m`);
      }

      const agentChatHistory = agent.saveChat ? await this.storage.fetchChat(userId, sessionId, agent.id) : [];

      const response = await agent.processRequest(content, userId, sessionId, agentChatHistory, additionalParams);
      ...
      if (agent.saveChat) {
        ...
        await this.storage.saveChatMessage(userId, sessionId, agent.id, userMessage);
        await this.storage.saveChatMessage(userId, sessionId, agent.id, assistantMessage);
      }
      ...
      .reduce<string[]>((acc, msg, i) => {
        if (i % 2 === 0 && i + 1 < agentsHistory.length) {
          ...
          if (!asstText.includes(this.id)) {
            acc.push(`${userMsg.role}:${userMsg.content?.[0]?.text || ""}\n` + `${asstMsg.role}:${asstText}\n`);
          }
        }
        return acc;
      }, [])
  ...
  async processRequest(
    ...
    try {
      ...
      this.leadAgent.setSystemPrompt(this.promptTemplate.replace("{{AGENTS_MEMORY}}", agentsMemory));

      return await this.leadAgent.processRequest(inputText, userId, sessionId, chatHistory, additionalParams);
    } catch (error) {
  ...
  getModel(): string {
    return this.leadAgent.getModel();
  }
