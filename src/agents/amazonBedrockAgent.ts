...
export class AmazonBedrockAgent extends Agent {
  ...
  constructor(options: AmazonBedrockAgentOptions) {
    ...
    this.client = options.client ? options.client : options.region ? new BedrockAgentRuntimeClient({ region: options.region }) : new BedrockAgentRuntimeClient();
  ...
  getModel(): string {
    return Agent.MODEL_NOT_AVAILABLE;
  }
